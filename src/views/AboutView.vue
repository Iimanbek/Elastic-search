  ...
  <Layout>
    <div class="about">
      <div>
        ...
        <v-spacer></v-spacer>
        <v-text-field v-model="searchValueAbout" @input="searchInputValue" label="Search... "
          prepend-inner-icon="mdi-magnify" hide-details="auto"></v-text-field>
        <v-select v-model="value" :items="items" @click:append="testing" chips label="Chips" multiple></v-select>
      </div>
...
export default {
  ...
  methods: {
    async defaultData() {
      ...
      this.result = await data.filter(item => item.id <= 51)
